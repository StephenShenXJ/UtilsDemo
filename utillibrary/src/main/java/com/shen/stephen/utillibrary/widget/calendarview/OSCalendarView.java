...
import com.shen.stephen.utillibrary.PKIApplication;
import com.shen.stephen.utillibrary.R;
import com.shen.stephen.utillibrary.adapter.RecyclePagerAdapter;
...
public class OSCalendarView extends FrameLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        Date date = PkiTimeUtil.parseDate(minDateStr);
        if (date == null) {
            date = PkiTimeUtil.parseDate(DEFAULT_MIN_DATE);
        }
        if(date!=null){
        ...
        date = PkiTimeUtil.parseDate(maxDateStr);
        if (date == null) {
            date = PkiTimeUtil.parseDate(DEFAULT_MAX_DATE);
        }
        if(date!=null){
        ...
        mDateTextSize = a.getDimensionPixelSize(R.styleable.OSCalendarView_dateTextSize, FileUtils.dip2px(PKIApplication.getContext(), DEFAULT_DATE_TEXT_SIZE));
        ...
        PkiTimeUtil.changeToStartOfDay(mSelectedDay);

        mMonthNumOfToday = PkiTimeUtil.getMonthsSinceDate(mMinDate, mTempDate);
        mWeekNumOfToday = PkiTimeUtil.getWeeksSinceDate(mMinDate, mTempDate, mFirstDayOfWeek);
                ...
                if (state == ViewPager.SCROLL_STATE_DRAGGING) {
                    ...
                    if (mFocusedWeekView != null && mCurrentCalendarMode == CALENDAR_VIEW_MODE_WEEK) {
                        ...
                        if (mSelectedDay.get(Calendar.MONTH) != mFocusedMonthView.mMonth) {
                            int newItemIndex = PkiTimeUtil.getMonthsSinceDate(mMinDate, mTempDate);
            ...
            public void onPageScrollStateChanged(int state) {
                ...
                if (state == ViewPager.SCROLL_STATE_DRAGGING) {
                    ...
                    if (mCurrentCalendarMode == CALENDAR_VIEW_MODE_MONTH) {
                        ...
                        final int weekNum = PkiTimeUtil.getWeeksSinceDate(mMinDate, mSelectedDay, mFirstDayOfWeek);
    ...
    protected void onRestoreInstanceState(Parcelable state) {
        ...
        setMonthDisplayed(mSelectedDay);
        daySelected(mSelectedDay);
        mMonthViewPager.setCurrentItem(PkiTimeUtil.getMonthsSinceDate(mMinDate, mSelectedDay), false);
        mWeekViewPager.setCurrentItem(PkiTimeUtil.getWeeksSinceDate(mMinDate, mSelectedDay, mFirstDayOfWeek), false);
    }
    ...
    protected boolean singleTapUp(MotionEvent e) {
        if (mMonthViewPager.isEnabled()) {
            ...
            if (mCurrentCalendarMode == CALENDAR_VIEW_MODE_MONTH) {
                ...
                final int weekNumber = PkiTimeUtil.getWeeksSinceDate(mMinDate, mTempDate, mFirstDayOfWeek);
    ...
    private void daySelected(Calendar selectedDay) {
        //OSLog.e("On Day " + PkiTimeUtil.formatDate(day.getTimeInMillis(), "yyyy/MM/dd") + " selected");
        ...
        if (PkiTimeUtil.isSameDay(mSelectedDay, mToday)) {
    ...
    public void setSelectedDay(Calendar selectedDay, boolean isAnimate) {
        ...
        final int weekNum = PkiTimeUtil.getWeeksSinceDate(mMinDate, mSelectedDay, mFirstDayOfWeek);
        final int monthNum = PkiTimeUtil.getMonthsSinceDate(mMinDate, mSelectedDay);
        if (mCurrentCalendarMode == CALENDAR_VIEW_MODE_WEEK) {
    ...
    public void setToday(long todayInMilliseconds) {
        ...
        if (!PkiTimeUtil.isSameDay(mTempDate, mToday)) {
    ...
    private void smoothToMonthMode() {
        final float end = mCurrentCalendarMode == CALENDAR_VIEW_MODE_MONTH ? -mScrollingDistance : getMaxScrollOffset() - mScrollingDistance;
        mCalendarMonthViewContainer.clearAnimation();
        Animation animation = new SmoothToMonthModeAnimator(0, (int)end, CALENDAR_VIEW_MODE_MONTH);
    ...
    private class WeeksAdapter extends RecyclePagerAdapter {
        ...
        protected void init() {
            ...
            mTotalCount = PkiTimeUtil.getWeeksSinceDate(mMinDate, mMaxDate, mFirstDayOfWeek);
    ...
    private class MonthAdapter extends WeeksAdapter {
        ...
        protected void init() {
            ...
            mTotalCount = PkiTimeUtil.getMonthsSinceDate(mMinDate, mMaxDate);
    ...
    protected class WeekView extends View {
        ...
        public WeekView(Context context) {
            ...
            mEventFlagRadius = FileUtils.dip2px(context, EVENT_FLAG_RADIUS);
            mEventFlagPadding = FileUtils.dip2px(context, EVENT_FLAG_PADDING);
            mDefaultPadding = FileUtils.dip2px(context, DEFAULT_PADDING);
        ...
        public void init(int weekNumber) {
            ...
            if (mWeekNumber != mWeekNumOfToday) {
                if (PkiTimeUtil.getWeeksSinceDate(mMinDate, mSelectedDay, mFirstDayOfWeek) == mWeekNumber) {
        ...
        protected void drawDayCell(Canvas canvas, int index, int divisor, int y) {
            ...
            if (mSelectedDayIndex == index) {
                ...
                mDayDrawPaint.setColor(PkiTimeUtil.isSameDay(mToday, mTempDate) ? mTodayBackgroundColor : mFocusedMonthDateColor);
            }
        ...
        protected void drawSelectedBackground(Canvas canvas, int x, int y, float radius) {
            mDrawPaint.setColor(PkiTimeUtil.isSameDay(mToday, mTempDate) ? mTodayBackgroundColor : mSelectedDayBackgroundColor);
    ...
    protected class MonthView extends WeekView {
        ...
        public void init(int monthNumber) {
            ...
            if (monthNumber != mMonthNumOfToday) {
                if (PkiTimeUtil.getMonthsSinceDate(mMinDate, mSelectedDay) == monthNumber) {
        ...
        protected void onDateTapped(Calendar selectedDay) {
            ...
            // OSLog.e("--------- Selected day(" + mSelectedDayIndex + ")------------------" + PkiTimeUtil.formatDate(selectedDay.getTimeInMillis(), "yyyy/MM/dd"));
            updateSelectionPositions();
            invalidate();
        }
