package com.shen.stephen.utillibrary.util;
...
public class StrUtil {
	...
	/**
	 * get the string from resource against the id in string format
	 * typically it is used to display the localized display name of enum
	 *
	 * @param context
	 * @param idString the id in string format
	 * @return the string
	 */
	public static String getResourceStringByIdString(Context context, String idString){
		int idOfString = context.getResources().getIdentifier(idString, "string", context.getPackageName());
		if ( idOfString > 0 ) {
	...
	public static String trimLeadingZero(String string){
		if(!isEmpty(string)){
